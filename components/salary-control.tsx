...
async function saveSalary(value: number, currency: string) {
  await fetch("/api/settings", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ salary: value, currency }),
  });
}
...
export function SalarySetupCard({ currency }: { currency: string }) {
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex flex-wrap items-start justify-between gap-6">
        ...
        <SalaryForm currency={currency} autoFocus={false} />
      </div>
    </div>
  );
}
...
export function SalaryChip({ salary, currency }: { salary: number; currency: string }) {
  const [editing, setEditing] = useState(false);
  if (editing) return <SalaryForm initial={salary} currency={currency} onDone={() => setEditing(false)} autoFocus />;
  return (
